...
func alice(sharedParams string, sharedG []byte, messageChannel chan *messageData, keyChannel chan []byte, keyChannel2 chan []byte, keyChannel3 chan []byte, finished chan bool) {
    ...
    // signature[0] = pairing.NewG2().PowZn(h, privKey)
    // signature[1] = pairing.NewG2().PowZn(h, private2)
    // signature[2] = pairing.NewG2().PowZn(h, private3)
...
// func aggregate(signature1 *pbc.Element, signature2 *pbc.Element) *pbc.Element {
//   aggregate_signature := pairing.NewG2().Add(signature1, signature2)
